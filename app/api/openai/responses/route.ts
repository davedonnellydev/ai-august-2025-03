...
export async function POST(request: NextRequest) {
  try {
    ...
    const tools = [
      {
        ...
        description:
          "Translate the user's query into a set of parameters in preparation for a call to a recipe API.",
        parameters: {
          type: 'object',
          properties: {
            query: {
            ...
            cuisine: {
              ...
              description:
                "If any of the cuisine values are mentioned in the user's query, add each of those cuisine values found to the array. Otherwise this can be an empty array.",
            },
            excludeCuisine: {
              type: 'array',
              items: {
              ...
              description:
                "If any of the cuisine values are mentioned in the user's query as a cuisine that should not be included in the search, add each of those cuisine values to the array. Otherwise this can be an empty array.",
            },
            diet: {
              type: 'array',
              items: {
              ...
              description:
                "If any of the diet values are mentioned in the user's query as a diet for which the recipes must be suitable, add each of those diet values found to the array. Otherwise this can be an empty array.",
            },
            intolerances: {
              type: 'array',
              items: {
              ...
              description:
                "If any of the intolerance values are mentioned in the user's query as an intolerance for which the recipes must account, add each of those intolerance values found to the array. Otherwise this can be an empty array.",
            },
            includeIngredients: {
              type: 'array',
              items: {
              ...
              description:
                "If the user mentions any ingredients to be included in the recipe, add each of those ingredients to the array as a separate value. If no ingredients are mentioned in the user's query, this can be an empty array.",
            },
            excludeIngredients: {
              type: 'array',
              items: {
              ...
              description:
                "If the user mentions any ingredients to be excluded from the recipe, add each of those ingredients to the array as a separate value. If no ingredients are mentioned in the user's query, this can be an empty array.",
            },
            type: {
              type: ['string', 'null'],
              description:
                "If any of the type values are mentioned in the user's query, assign that value as the type. If no type is mentioned, this can be a null value.",
              enum: [
                'main',
                'course',
                'side',
            ...
            maxReadyTime: {
              type: ['number', 'null'],
              description:
                "The maximum time in minutes it should take to prepare and cook the recipe. If no maxiumum time is mentioned in the user's query, this can be a null value.",
            },
          },
          $defs: {
            cuisine: {
    ...
    const instructions: string = `Your job is to convert the user's request into a query string to use with the Spoonacular API call. The query string should start with a '?' and contain any appropriate key value pairs in the form of a url query string e.g.:
